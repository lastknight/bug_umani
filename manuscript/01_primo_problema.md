# Problema 1: Abbiamo troppe informazioni.
...
- [Humor effect](https://en.wikipedia.org/wiki/List_of_memory_biases): fa sì che le persone ricordino meglio le informazioni quando le percepiscono come umoristiche. Questo può essere dovuto al carattere distintivo del messaggio umoristico, ma anche al tempo necessario alla sua comprensione e al risveglio emotivo che provoca;
- [Von Restorff effect](https://en.wikipedia.org/wiki/Von_Restorff_effect): prevede che quando vengono presentati più stimoli omogenei, lo stimolo che differisce dagli altri ha più probabilità di essere ricordato. L'effetto risulta essere più efficace se l'elemento che differisce viene presentato in qualche modo isolato dagli altri (isolation paradigm);
- [Negativity bias](https://en.wikipedia.org/wiki/Negativity_bias): a parità di intensità, le cose di natura più negativa (ad esempio, pensieri sgradevoli, emozioni o interazioni sociali, eventi dannosi/traumatici) hanno un effetto maggiore sul proprio stato psicologico e sui propri processi rispetto alle cose neutre o positive. Questo è sicuramente dovuto al fatto che gli elementi negativi vengano solitamente considerati più importanti e che il concetto di "negativo" sia più elaborato del concetto di "positivo", cosa che si evince anche dal fatto che il vocabolario negativo è maggiormente descrittivo di quello positivo;
- [Publication bias](https://en.wikipedia.org/wiki/Publication_bias): euristica tipica dell'ambito delle pubblicazioni accademiche che si verifica quando il risultato di un esperimento o di uno studio di ricerca influenza la decisione di pubblicarlo o meno, o di distribuirlo in altro modo. Per questo motivo, statisticamente i paper con risultati significativi vengono pubblicati tre volte di più rispetto a paper con esiti minori, provocando uno sbilanciamento;
- [Omission bias](https://en.wikipedia.org/wiki/Omission_bias): è la tendenza a privilegiare un atto di omissione rispetto ad uno di commissione, ovvero evidenzia la propensione umana a non agire piuttosto che a compiere un'azione. L'ambito in cui è maggiormente evidente è quello morale, tanto che un'azione immorale viene percepita come più sbagliata rispetto a un'omissione immorale.

## Notiamo principalmente quando le cose cambiano.

Tendiamo a valutare quanto significativa è un'informazione sulla base del cambiamento (positivo o negativo) che ha avuto su di noi, piuttosto che sul valore intrinseco dell’evento. Tendiamo anche a effettuare valutazioni su come qualcosa impatterà sul nostro futuro piuttosto che acriticamente e usiamo lo stesso metro anche per effettuare paragoni.

*Riferimenti:*

- [Anchoring](https://en.wikipedia.org/wiki/Anchoring):  bias per cui un individuo dipende da un'informazione iniziale (l'ancora) nel prendere decisioni o esprimere giudizi successivi. Una volta impostato il valore dell'ancora, infatti, ogni stima effettuata dall'utente viene condizionata da essa. Alcuni dei fattori che aumentano le probabilità di incorrere nel bias sono l'umore (una persona felice ne è più soggetta, dato che la tristezza è legata a una valutazione più accurata dei problemi), l'esperienza (una bassa expertise nell'ambito in cui è necessario effettuare una stima, porta ad "ancorarsi" maggiormente), la personalità (secondo alcuni studi, le persone più aperte a nuove esperienze sono più soggette al bias), l'abilità cognitiva (una minore abilità cognitiva porta ad affidarsi maggiormente all'ancora presentata) e l'eccesso di fiducia in sè stessi.
- [Contrast effect](https://en.wikipedia.org/wiki/Contrast_effect): è l'aumento o la diminuzione, rispetto al normale, della percezione, della cognizione o delle prestazioni correlate, come risultato dell'esposizione successiva (immediatamente precedente) o simultanea ad uno stimolo di valore minore o maggiore nella stessa dimensione. Per questa ragione, un elemento di colore grigio apparirà più chiaro o più scuro se immediatamente preceduto o presentato contemporaneamente ad un altro elemento più scuro o più chiaro;
- [Focusing effect](https://pubmed.ncbi.nlm.nih.gov/16919591/): si verifica quando una persona dà troppa importanza a un dettaglio selezionato piuttosto che considerare il "quadro generale" - questo può portare a errori nella previsione di un risultato futuro;
- [Framing effect](https://en.wikipedia.org/wiki/Framing_effect_%28psychology%29):  bias in cui le persone scelgono tra diverse opzioni in base al fatto che queste sono state presentate con connotazioni positive o negative. Per questa ragione l'individuo tende a evitare rischi quando il quadro che gli viene offerto è positivo e a cercarli quando invece è piuttosto negativo;
...
Weber afferma che "l'aumento minimo dello stimolo che produrrà un aumento percettibile della sensazione è proporzionale allo stimolo preesistente", mentre la legge di Fechner è un'inferenza dalla legge di Weber che afferma che "l'intensità della nostra sensazione aumenta solo come il logaritmo di un aumento di energia, piuttosto che rapidamente come l'aumento";
- [Distinction bias](https://en.wikipedia.org/wiki/Distinction_bias): è la tendenza a considerare due opzioni come più distintive quando le si valuta contemporaneamente che quando le si valuta separatamente. Questo perché la comparazione rende l'individuo più propenso a esaminare e valutare esageratamente le differenze tra le alternative presenti.
